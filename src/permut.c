...
SEXP permut(SEXP p, SEXP ryx, SEXP us, SEXP t2, SEXP details, SEXP permute, SEXP step, SEXP ddfr)
{
  ...
  SEXP p3, p4, ryx1, ryx2, tus, pmus, uspyxi1, uspyxi2, tp3, tp4;
  PROTECT_INDEX ip3, ip4, iryx1, iryx2, itus, ipmus , iuspyxi1, iuspyxi2, itp3, itp4;
  double t2cpy, t2icpy;
  ...
  PROTECT_WITH_INDEX(tp3 = allocVector(REALSXP, 1), &itp3);
  PROTECT_WITH_INDEX(tp4 = allocVector(REALSXP, 1), &itp4);
  ...
  for (i = 0 ; i < REAL(permute)[0] ; i++)
    {
      ...
      REPROTECT(tp3 = transposeMat(p3), itp3);
      REPROTECT(p3 = prodMat(tp3, p3), ip3);
      REPROTECT(p4 = selectCol(p34, 2), ip4);
      REPROTECT(tp4 = transposeMat(p4), itp4);
      REPROTECT(p4 = prodMat(tp4, p4), ip4);
      ...
      t2cpy = fabs(REAL(t2)[0]);
      t2icpy = REAL(t2i)[0];
      j = (R_len_t)REAL(step)[0] - 1;
      if (t2icpy <= -t2cpy)
	REAL(tmp3)[j]++;
      else
	{
